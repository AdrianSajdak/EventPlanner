...
const TextField = ({
  ...
  <TextInput
    ...
    style={[styles.input, value ? styles.inputFilled : styles.inputEmpty, multiline && styles.textArea]}
    multiline={multiline}
    autoCorrect={false}
    spellCheck={false}
  />
...
export default function EventEditorScreen({ navigation, route }: Props) {
  ...
  const [title, setTitle] = useState('Wieczorne Kino w Plenerze');
  const [location, setLocation] = useState(event.locationName || 'Cybermachina');
  ...
  const handleSave = () => {
    const newLabel = [date.trim(), time.trim()].filter(Boolean).join(', ');
    updateEvent(eventId, {
      title: title.trim() || event.title,
      dateLabel: newLabel || event.dateLabel,
      shortDate: newLabel || event.shortDate,
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.safe}>
        <Navbar title="Edytuj wyjście" showBack onBack={() => navigation.goBack()} />
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={styles.cancelButtonText}>Odwołaj wydarzenie</Text>
          ...
          <TouchableOpacity style={styles.saveButton} onPress={handleSave} activeOpacity={0.85}>
            <Text style={styles.saveButtonText}>Zapisz edycję</Text>
